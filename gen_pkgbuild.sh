...
envsubst '${daemon_name} ${srv_id}' <"${TMPLDIR}/server.install" > "${BASEDIR}/${srv_type}-server.install"
envsubst '${srv_type} ${mc_ver} ${daemon_name} ${srv_id}' <"${TMPLDIR}/backup.service" > "${BASEDIR}/${daemon_name}-backup.service"
envsubst '${srv_type} ${mc_ver}' <"${TMPLDIR}/backup.timer" > "${BASEDIR}/${daemon_name}-backup.timer"
envsubst '${srv_id}' <"${TMPLDIR}/d.conf" > "${BASEDIR}/${daemon_name}.conf"
envsubst '${mc_ver} ${srv_type} ${daemon_name} ${srv_id}' <"${TMPLDIR}/d.service" > "${BASEDIR}/${daemon_name}.service"
envsubst '${daemon_name} ${srv_id}' <"${TMPLDIR}/d.sh" > "${BASEDIR}/${daemon_name}.sh"
envsubst '${mc_ver} ${srv_type} ${srv_id}' <"${TMPLDIR}/d.sysusers" > "${BASEDIR}/${daemon_name}.sysusers"
envsubst '${srv_id}' <"${TMPLDIR}/d.tmpfiles" > "${BASEDIR}/${daemon_name}.tmpfiles"
